...
def test_error(http):
    ...
    res = http.patch_json('/series/state', params={
        'name': 'test-error',
        'series': json.loads(util.tojson(v2)),
        'author': 'Babar',
        'tzaware': util.tzaware_series(v2)
    })
    assert res.status_code == 418
    # the error is produced in http/server with the line
    # series = pd.Series(args.series, dtype=dtype)
    # when the json series is coerced in another type
    assert res.body == (
        b"could not convert string to float: 'a'"
    )
...
""", series)
    ...
    res = http.patch_json('/series/state', params={
        ...
        'series': json.loads(util.tojson(series_in)),
        'author': 'Babar',
        'insertion_date': str(utcdt(2018, 1, 1, 13)),
        'tzaware': util.tzaware_series(series_in)
    })
...
""", df)
    ...
    meta, hist = codecs.unpack_history(res.body)
    assert_hist("""
...
""", df)
    ...
    res = http.patch_json('/series/state', params={
        ...
        'series': json.loads(util.tojson(series_in)),
        'author': 'Babar',
        'insertion_date': str(utcdt(2018, 1, 2)),
        'tzaware': util.tzaware_series(series_in),
        'replace': True
    })
    ...
    assert res.json == {
        '2019-01-01T00:00:00+00:00': 0.0,
        '2019-01-01T01:00:00+00:00': 1.0,
        '2019-01-01T02:00:00+00:00': 2.0
    }
...
def test_patch_nonutc_tzaware(http):
    ...
    res = http.patch_json('/series/state', params={
        ...
        'tzone': 'Europe/Paris',
        'series': util.tojson2(ts),
        'author': 'Babar',
        'insertion_date': str(utcdt(2023, 1, 1)),
        'tzaware': util.tzaware_series(ts)
    })
    assert res.status_code == 201
...
def test_multisource(http, engine):
    ...
    assert out.json == {
        ...
        '2020-01-02T00:00:00+00:00': 1.0,
        '2020-01-03T00:00:00+00:00': 2.0
    }
...
def test_log(http):
    ...
    for d in range(5):
        res = http.patch_json('/series/state', params={
            ...
            'series': json.loads(util.tojson(series)),
            'author': 'Babar',
            'insertion_date': str(utcdt(2020, 1, d + 1)),
    ...
    assert out.json == {
        '2020-01-01T00:00:00+00:00': 42.0,
        '2020-01-02T00:00:00+00:00': 1.0,
        '2020-01-03T00:00:00+00:00': 2.0,
        '2020-01-04T00:00:00+00:00': 3.0,
        '2020-01-05T00:00:00+00:00': 4.0
    }
...
def test_tzaware_json_group(http):
    ...
    assert res.json == {
        'a': {
            '2021-01-01T00:00:00+00:00': 2.0,
            '2021-01-02T00:00:00+00:00': 3.0,
            '2021-01-03T00:00:00+00:00': 4.0,
            '2021-01-04T00:00:00+00:00': 5.0,
            '2021-01-05T00:00:00+00:00': 6.0
        },
        'b': {
            '2021-01-01T00:00:00+00:00': 3.0,
            '2021-01-02T00:00:00+00:00': 4.0,
            '2021-01-03T00:00:00+00:00': 5.0,
            '2021-01-04T00:00:00+00:00': 6.0,
            '2021-01-05T00:00:00+00:00': 7.0
        },
        'c': {
            '2021-01-01T00:00:00+00:00': 4.0,
            '2021-01-02T00:00:00+00:00': 5.0,
            '2021-01-03T00:00:00+00:00': 6.0,
            '2021-01-04T00:00:00+00:00': 7.0,
